...
INSERT @oldAgentGuids(agentGuid) 
	SELECT * from (
		...
		--,('4E7DBF83-C74A-462F-BF9A-4B4360ADBB42') -- TEMP TEST ONLY
		) AS TempTable([Guid])

--DECLARE @oldAgentGuid UNIQUEIDENTIFIER = '4E7DBF83-C74A-462F-BF9A-4B4360ADBB42'
...
SELECT * FROM [EDDSDBO].[ApplicationGuid]
	JOIN @oldAgentGuids o ON [ArtifactGuid] = o.agentGuid
SELECT * FROM [EDDSDBO].[ApplicationAgentType] 
	JOIN @agentTypeIds ati ON [AgentTypeArtifactID] = ati.agentTypeId
SELECT * FROM [EDDSDBO].[AgentType] 
	JOIN @oldAgentGuids o ON [AgentTypeGuid] = o.agentGuid