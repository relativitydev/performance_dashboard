namespace roundhouse.infrastructure.app.builders
{
    ...
    using filesystem;
    using loaders;
    using roundhouse.databases.sqlserver2;
    ...
    public static class DatabaseBuilder
    {
        ...
        public static Database build(FileSystemAccess file_system, ConfigurationPropertyHolder configuration_property_holder)
        {
            ...
            try
            {
                ...
                database_to_migrate = new SqlServerDatabase(); //DefaultInstanceCreator.create_object_from_string_type<Database>(database_type + ", " + merge_assembly_name);

            }
            catch (NullReferenceException)
            {
                database_to_migrate = new SqlServerDatabase();//DefaultInstanceCreator.create_object_from_string_type<Database>(configuration_property_holder.DatabaseType);
            }
