...
BEGIN
	IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[eddsdbo].[QoS_BackResults]') AND name = N'IX_LoggedDate')
	BEGIN
		...
		SET @SQL = 'CREATE NONCLUSTERED INDEX [IX_LoggedDate] ON [eddsdbo].[QoS_BackResults]
		(
			[LoggedDate] ASC
		)
		'		
