...
BEGIN
	IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[eddsdbo].[QoS_DBCCHistory]') AND name = N'IX_CompletedOn')
	BEGIN
		...
		SET @SQL = 'CREATE NONCLUSTERED INDEX [IX_CompletedOn] ON [eddsdbo].[QoS_DBCCHistory]
		(
			[CompletedOn] ASC
		)
		'		
