...
BEGIN
	IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[eddsdbo].[QoS_SampleHistory]') AND name = N'NCI_IsActiveWeekly4Sample')
	BEGIN
		...
		SET @SQL = 'CREATE NONCLUSTERED INDEX [NCI_IsActiveWeekly4Sample] ON [eddsdbo].[QoS_SampleHistory] 
	(
		[IsActiveWeekly4Sample] ASC
	)
