...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[eddsdbo].[QoS_BackupAndDBCCCheckMonLauncher]') AND type in (N'P', N'PC'))
DROP PROCEDURE [eddsdbo].[QoS_BackupAndDBCCCheckMonLauncher]
GO
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[eddsdbo].[QoS_DBInfoBackResults]') AND type in (N'U'))
DROP TABLE EDDSDBO.QoS_DBInfoBackResults
GO
...
CREATE PROCEDURE [eddsdbo].[QoS_BackupAndDBCCCheckMonLauncher]
...
BEGIN
	...
	DECLARE
		...
		@Module NVARCHAR(100) = 'QoS_BackupAndDBCCCheckMonLauncher',
		@lastDate datetime,
	...
	BEGIN
		...
		EXEC EDDSPerformance.eddsdbo.QoS_LogAppend
			...
			@otherVars = @loggingVars,
			@nextTask = 'Create History, Populating EDDSDBO.QoS_tempServers table, and truncate the tempServers table.'
	END
	...
	--	Get list of servers
	--
	--	Insert list of server in the environment into the EDDSDBO.QoS_tempServers table
	...
	INSERT INTO EDDSDBO.QoS_tempServers 
	...
	BEGIN
		...
		EXEC EDDSPerformance.eddsdbo.QoS_LogAppend
			@AgentID = @AgentID,
			@module = @Module,
			@taskCompleted = 'Inserted servers into tempServers table ',
			@otherVars = @loggingVars,
	...
	DECLARE @ID INT = (SELECT MIN(ServerID) FROM EDDSDBO.QoS_tempServers)
	DECLARE @MaxID INT = (SELECT MAX(ServerID) FROM EDDSDBO.QoS_tempServers)
	...
	WHILE(@ID <= @MaxID)
	BEGIN
		-- Work on the top 1 server from the EDDSDBO.QoS_tempServers table	
		SET @tempServer = (SELECT TOP 1 ServerName from EDDSDBO.QoS_tempServers WHERE ServerID = @Id)
		...
		-- Call inside sproc with top 1 serverName	
		SET @USESQL = '
			EXECUTE ' + QUOTENAME(@tempServer) + '.[EDDSQoS].[eddsdbo].[QoS_DBCCCheckServerMon]
			@beginDate = @beginDate
			,@enddate = @endDate
		...
		declare @USESQLBackup nvarchar(max);
		SET @USESQLBackup = '
			EXECUTE ' + QUOTENAME(@tempServer) + '.[EDDSQoS].[eddsdbo].[QoS_BackupCheckServerMon]
			@summaryDayHour = @summaryDayHour
			,@currentServerName = @tempServer
			,@eddsPerformanceServerName = @eddsPerformanceServerName'
		...
		BEGIN TRY
			exec sp_executesql @USESQL, N'@beginDate DATETIME,
					@endDate DATETIME,
					...
					@useDbccDbinfo bit,
					@logging bit',
					@beginDate, @endDate, @summaryDayHour, @AgentID, @tempServer, @eddsPerformanceServerName, @eddsServerName, @lastDate, @useDbccDbinfo, @logging
			exec sp_executesql @USESQLBackup, N'@summaryDayHour datetime, @tempServer nVARCHAR(255), @eddsPerformanceServerName nvarchar(255)',
					@summaryDayHour, @tempServer, @eddsPerformanceServerName
		END TRY
		BEGIN CATCH
			...
			UPDATE EDDSDBO.QoS_tempServers
	...
	BEGIN
		...
		EXEC EDDSPerformance.eddsdbo.QoS_LogAppend
			...
			@otherVars = @loggingVars,
			@nextTask = 'Checking view-based monitoring status'
	END
	...
	BEGIN
		EXEC EDDSPerformance.eddsdbo.QoS_LogAppend
			...
			@nextTask = 'Checking for redundant items in database list'
	END
	
	--If we found the same database on multiple servers, only keep the one from the current server
	DELETE R
	FROM [EDDSPerformance].[eddsdbo].[QoS_AllDatabasesChecked] R
	LEFT JOIN [EDDS].[eddsdbo].[ExtendedCase] E WITH(NOLOCK)
	ON R.CaseArtifactID = E.ArtifactID
	AND R.ServerName = E.DBLocation
	WHERE R.CaseArtifactID > 0
	AND E.ArtifactID IS NULL
	...
	BEGIN
		EXEC EDDSPerformance.eddsdbo.QoS_LogAppend
			@AgentID = @AgentID,
			@module = @Module,
			@taskCompleted = 'Checked for redundant items in database list',
	...
	BEGIN
		EXEC EDDSPerformance.eddsdbo.QoS_LogAppend
			...
			@nextTask = 'Excluding deleted workspaces from results'
	END
	
	--Erase history for databases that have been removed from Relativity
	DELETE br
	FROM eddsdbo.QoS_BackResults br
	LEFT JOIN EDDS.eddsdbo.[Case] c WITH(NOLOCK)
	ON br.CaseArtifactID = c.ArtifactID
	WHERE br.CaseArtifactID > 0 AND c.ArtifactID IS NULL

	DELETE dr
	FROM eddsdbo.QoS_DBCCResults dr
	LEFT JOIN EDDS.eddsdbo.[Case] c WITH(NOLOCK)
	ON dr.CaseArtifactID = c.ArtifactID
	WHERE dr.CaseArtifactID > 0 AND c.ArtifactID IS NULL

	DELETE bs
	FROM eddsdbo.QoS_BackSummary bs
	LEFT JOIN EDDS.eddsdbo.[Case] c WITH(NOLOCK)
	ON bs.CaseArtifactID = c.ArtifactID
	WHERE bs.CaseArtifactID > 0 AND c.ArtifactID IS NULL

	DELETE ds
	FROM eddsdbo.QoS_DBCCSummary ds
	LEFT JOIN EDDS.eddsdbo.[Case] c WITH(NOLOCK)
	ON ds.CaseArtifactID = c.ArtifactID
	WHERE ds.CaseArtifactID > 0 AND c.ArtifactID IS NULL
	...
	BEGIN
		EXEC EDDSPerformance.eddsdbo.QoS_LogAppend
			@AgentID = @AgentID,
			@module = @Module,
			@taskCompleted = 'Excluded deleted workspaces from results',
	...
	LEFT JOIN EDDS.eddsdbo.[Case] c WITH(NOLOCK)
		ON c.ArtifactID = g.CaseArtifactID
