...
namespace kCura.PDD.Web
{
	...
	public class UserExperienceController : ApiController
	{
		...
		public string HoursLinkFormat = @"<a href=""/Relativity/CustomPages/60a1d0a3-2797-4fb3-a260-614cbfd3fa0d/UserExperienceHours.aspx?Server={0}{1}"">{2}</a>";
		public string SearchLinkFormat = @"<a href=""/Relativity/CustomPages/60a1d0a3-2797-4fb3-a260-614cbfd3fa0d/UserExperienceSearch.aspx?CaseArtifactID={0}{1}"">{2}</a>";
			...
			var dataSets = (from score in scores
											...
											select new ScoreChartDataSet
											{
												...
												data = from hour in hours select (int?)
														...
														?? new ScoreChartModel()).UserExperienceScore,
												sample = from hour in hours select (bool?)
														(serverScores.FirstOrDefault(x => x.SummaryDayHour.Equals(hour))
